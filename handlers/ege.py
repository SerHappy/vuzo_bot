...
from utils.excel_utils import create_xlsx_directions
...
async def start_fsm_for_subject(
...
async def process_amount_invalid(message: types.Message) -> None:
    ...
    # Формирование сообщения бота
    await message.answer(
        "Количество предметов дожно быть в диапозоне от 3 до "
...
async def process_amount(message: types.Message, state=FSMContext) -> None:
    ...
    # Формирование сообщения бота
    await message.answer(
        "Выбери предмет, который ты сдавал:", reply_markup=keyboard
    )
...
async def process_score(message: types.Message, state=FSMContext) -> None:
    ...
    if get_association_user_subject_records_count_by_user_id(
        message.from_user.id
    ) < get_User_ege_subjects_number(message.from_user.id):
        # Установка состояния в "ожидание ввода предмета,
        ...
        # if "Математика (профиль)" in user_scores
...
async def process_individual_archivments_invalid(
...
async def process_individual_archivments(
    ...
    answer = (
        "Текущие введенные значения:\nКоличество предметов:"
        f" {get_User_ege_subjects_number(message.from_user.id)}\n"
    )
    total_score = 0
    for ege_record in get_association_user_subject_all_records_by_id(
        message.from_user.id
    ):
        # Добавление в переменную answer информации о предмете
        ege_record_subject_name = get_subject_name_by_id(ege_record.subject_id)
        answer += (
            f"Предмет '{ege_record_subject_name}', баллов -"
            f" {ege_record.score}\n"
        )
        # Прибавл. к переменной общих баллов кол-ва баллов за предмет
        total_score += ege_record.score
        print(total_score, state)
    indv_record = get_association_user_subject_indv_record(
        message.from_user.id
    )
    if indv_record is not None:
    ...
    # f"{individual_achievements_value}"
    #     # Прибавление к переменной общих баллов кол-ва баллов за ИД
    #     total_score += individual_achievements_value
    answer += (
        "\nОбщая сумма баллов равна: "
        f"{get_User_ege_total_score(message.from_user.id)}"
    )
    # Формирование шаблонов сообщений
...
async def process_search_start(
    message: types.Message, state=FSMContext
) -> None:
    """
    ...
    if not find:
        ...
        await message.answer_document(
            open(
                os.path.join(
                    config("PROJECT_DIR"),
                    "data/parsing/mirea/Факультеты.xlsx",
                ),
                "rb",
            )
        )
